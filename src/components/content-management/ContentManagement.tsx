            ...
            <div className="flex items-center gap-1">
              ...
              {PAGES.map((p, i) => (
                p === "..." ? <span key={i} className="px-1 text-sm" style={{ color: "#6C757D" }}>...</span> :
                  <button key={i} onClick={() => setPage(Number(p))} className="w-8 h-8 text-sm rounded-lg font-medium transition-colors cursor-pointer" style={page === p ? { backgroundColor: "#FF4A00", color: "#FFFFFF" } : { color: "#2C2E33", backgroundColor: "transparent" }}>{p}</button>
              ))}
